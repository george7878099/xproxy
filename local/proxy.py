...
sys.path = [(os.path.dirname(__file__) or '.') + '/packages.egg/noarch'] + [(os.path.dirname(__file__) or '.') + '/packages.egg/' + sysconfig.get_platform().split('-')[0]] + sys.path
...
class GAEFetchPlugin(BaseFetchPlugin):
        ...
        for i in xrange(self.max_retry):
            try:
                ...
                if response.app_status < 500:
                    ...
                    elif response.app_status != 502 and i < self.max_retry - 1 and len(self.appids) > 1:
                        self.appids.append(self.appids.pop(0))
    ...
    def fetch(self, handler, method, url, headers, body, timeout, **kwargs):
        ...
        if handler.body_unread_len == 0 and isinstance(body, basestring) and body:
        ...
        if common.GAE_OBFUSCATE:
            ...
            if handler.body_unread_len > 0:
                while handler.body_unread_len > 0:
                    data = handler.body_unread.read(min(8192, handler.body_unread_len))
                    if not data:
                        break
                    handler.body += data
            ...
            if 'rc4' in common.GAE_OPTIONS:
                ...
                body = '%s%s%s' % (struct.pack('!h', len(payload)), payload, body)
                request_headers['Content-Length'] = str(len(body) + handler.body_unread_len)
        # post data
